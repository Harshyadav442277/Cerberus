...
| **Internal service exposure** | `app.listen(PORT)` bound `0.0.0.0` on both the control plane and the executor. | Both bind `127.0.0.1` by default via `resolveBindHost()`. Binding wider requires setting `API_BIND_HOST` / `EXECUTOR_BIND_HOST`, and the process prints a warning when it does. | Attack H — 7 assertions | **Fixed (deployment-boundary option A)** | This is the "loopback/private-network by design" option, not service-to-service authentication. **No public-internet safety is claimed.** An operator who binds wider is responsible for what sits in front. |
...
| **CI** | No GitHub status checks on head. | GitHub Actions workflow running PostgreSQL, migrations, role provisioning, typecheck, tests, adversarial, red team, dashboard build and contract compile — with no wallet secrets and no live payment. | — | **Implemented** | CI does not run a funded payment, by design. The live-evidence run stays manual. |
...
> Prototype trusted-service interfaces are loopback scoped.
